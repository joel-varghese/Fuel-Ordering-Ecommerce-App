...
const CardPayment = (props) => {
    ...
    const payload= {"blobname":process.env.REACT_APP_READ_FBO_SIGNUP_PAYMENT_BLOBNAME}
    const dataValue =[] ;

    useEffect(() => {
        bfaJsonService(payload).then(response=>{
            console.log("CC", response.data.operatorSignup)
    ...
    // console.log("result",addNewFiled[0].Id)
        ...
        const node = document.getElementById("cardpayemnt");
        const clone = node.cloneNode(true);
        console.log(clone);
        document.getElementById("addCard").appendChild(clone);
        ...
        const getOperatorFields = (item) => {
            console.log("field", item.component);  
