...
function BankDetailsForm() {
    ...
    const payload = { "blobname": process.env.REACT_APP_READ_FBO_SIGNUP_BANK_BLOBNAME }
    useEffect(() => {
    ...
    const handleDelete =(event) =>{
        ...
        const newContacts = operatorSignup;
        console.log(operatorSignup.sections[0].subSections[0].fields);
        const index =  operatorSignup.sections[0].subSections[0].fields.findIndex((operatorSignup) => operatorSignup.Id === 2);
        console.log('index', index)
        newContacts.sections[0].subSections[0].fields.splice(index, 1);
        console.log("newContacts", )
        setoperatorSignupForm(newContacts)
        event.preventDefault();
    }
    ...
    const handleAddFormSubmit = (event) => {
        event.preventDefault();
        const  addNewFiled ={}
        addNewFiled['fields'] = operatorSignup.sections[0].subSections[0].fields;
        operatorSignup.sections[0].subSections.push(addNewFiled)
        console.log("operatorSignup", operatorSignup);
        const newFieldRow = {
            "component": "row",
            "type":"row",
            "label":"",
...
//         console.log("operatorSignup.sections[0].subSections[0].fields", operatorSignup.sections[0].subSections[0].fields.length)

//         for (let i = 0; i <addNewFiledLenght; i++) {
//             operatorSignup.sections[0].subSections[0].fields[filedLenght + i] = addNewFiled[i]

//         }
//         console.log('newContacts', operatorSignup);
        ...
        <div >
            ...
            {isBusy ? (<p>Loading</p>) : (
                <Form>
                    ...
                    <div id="uploaddocument" className={`bf-section ${addNew ? 'bf-add-new-section' : ''}`}>
