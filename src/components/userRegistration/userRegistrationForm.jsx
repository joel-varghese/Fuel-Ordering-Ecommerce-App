...
function UserRegistrationForm(){
        ...
        verifyUserRegisteration(username).then( data => {
            ...
            if( data && data.data && data.data.body && data.data.body) {
               ...
               // let uname = username.split('=')
                console.log('result in user register form ', result)
                if( result === 'Yes' ) {
                    console.log('user exists')
                    setModalText('The link is no longer valid or has expired.' )
                    setModalShow(true);
    ...
    const getUserFields = (item) => {
        ...
        case "BUTTON":
            return (<Row className='mb-3'>{ ! loader ? <ButtonComponent 
                    Label={item.fieldLabel} 
                    Type={item.type}
                    className={item.className}
                    disabled={item.name === 'submitButton' ? button : false}
                    variant = {item.variant}
                    handleClick = {(e) => onClickSubmit(e)} />:<Loader  height='auto'/>}</Row>)
        case "LIST":
            return (
                    <>
    ...
    return (<>
        ...
        <div className={`d-flex d-flex-row login-section bf-user-registration-section ${modalShow?'bf-show-model-blr':''}`}>
            ...
            <div className='bf-font-14 user-reg-copyright'>{fetchUserdata.content.copyright.text}</div>       
